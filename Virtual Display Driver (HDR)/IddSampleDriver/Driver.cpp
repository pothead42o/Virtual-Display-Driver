...
Microsoft::IndirectDisp::IndirectDeviceContext::s_KnownMonitorEdid;
...
//Start of a log function, can be removed in release builds
void logEdid(const std::vector<BYTE>& edid, const std::string& logFilePath) {
...
const BYTE IndirectDeviceContext::s_edidtest[] = 
{
	...
	,
...
std::vector<BYTE> loadEdid(const std::string& filePath) {
	std::ifstream file(filePath, std::ios::binary | std::ios::ate);
	if (!file) {
		logEdid(hardcodedEdid, "C:\\IddSampleDriver\\hardvddlog.txt");
	...
	if (file.read((char*)buffer.data(), size)) {
		logEdid(buffer, "C:\\IddSampleDriver\\buffervddlog.txt");
		return buffer;
	}
	else {
		logEdid(hardcodedEdid, "C:\\IddSampleDriver\\hardcodedelsevddlog.txt");
		return hardcodedEdid;
	}
}

void modifyEdid(std::vector<BYTE>& edid) {
	if (edid.size() < 12) {
		return;
	}
...
BYTE calculateChecksum(const std::vector<BYTE>& edid) {
	...
	// Anything after the checksum bytes arent part of the checksum
}


//std::vector<BYTE> s_KnownMonitorEdid; Old global - Can be commented as switched to inside indirectDeviceContext Class defined in header

int maincalc() {
	std::vector<BYTE> edid = loadEdid("C:\\IddSampleDriver\\user_edid.bin");
	
	modifyEdid(edid);
	BYTE checksum = calculateChecksum(edid);
	edid[127] = checksum;

	// Set s_KnownMonitorEdid through the instance of IndirectDeviceContext Depricated. We reference straight to the class now instead of an instance
...
void IndirectDeviceContext::CreateMonitor(unsigned int index) {
	...
	//MonitorInfo.MonitorDescription.DataSize = sizeof(IndirectDeviceContext::s_KnownMonitorEdid);            Doesnt work              
	//MonitorInfo.MonitorDescription.DataSize = sizeof(IndirectDeviceContext::s_edidtest);					  works with test edid
	if (s_KnownMonitorEdid.size() > UINT_MAX)
	{
		// Handle error if if need be, we can put placeholder here
	}
	else
	{
	...
	MonitorInfo.MonitorDescription.pData = IndirectDeviceContext::s_KnownMonitorEdid.data();               // This works, we can keep this data conversion instead of a const cast - const cast not needed as its not a constant variable
	//MonitorInfo.MonitorDescription.pData = const_cast<BYTE*>(IndirectDeviceContext::s_edidtest);            this is not the issue (OldTester code
	logEdid(IndirectDeviceContext::s_KnownMonitorEdid, "C:\\IddSampleDriver\\vddlog.txt");

	std::ofstream logFileLive("C:\\IddSampleDriver\\vddlog.txt", std::ios::app);
	logFileLive << '\n' << "EdidSizeNewSizeFunction : " << IndirectDeviceContext::s_KnownMonitorEdid.size() << '\n';
	logFileLive << "EdidSizeNewStaticCast: " << static_cast<UINT>(s_KnownMonitorEdid.size()) << '\n';
	logFileLive << "EdidSizeOLD(Working): " << sizeof(IndirectDeviceContext::s_edidtest) << '\n';
	logFileLive.close();
